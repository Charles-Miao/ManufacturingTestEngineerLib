...
```
- FlashBiosTest，同上
- ParallelTest，这是一个并行测试管理器，负责同时运行多个硬件测试项目，协调它们的执行和监控测试状态
```C#
// 1. 初始化
// 1.1 结束气泡屏保程序避免干扰测试
// 1.2 根据SKU编号判断是否启用触屏测试
// 1.3 检查AC电源状态
// 1.4 获取窗口句柄并发送最小化命令
// 2. 启动并行测试模式
// 2.1 使用原生方法User32.dll，每15秒在60Hz和144Hz之间切换显示频率，测试时间结束后恢复默认频率(120Hz)
// 2.2 TAT电源限制设置，使用TAT工具TATSetPower.bat设置CPU电源限制，备份生成的日志文件
// 2.3 CPU烧机测试，启动 cpu_burnin.exe 进行CPU压力测试，测试时间根据配置计算，监控TAT日志分析CPU频率和功耗，测试结束后kill进程并备份日志
// 2.4 LCD运行测试，启动 PictureBox.exe 进行LCD显示测试，显示图像验证屏幕功能，测试结束后备份结果和调试日志
// 2.5 WIFI和蓝牙测试，分别启动WIFI和蓝牙运行测试工具WifiRunin.exe 和 BtRunin.exe，测试结束后备份结果和调试日志
// 2.6 音频测试，启动 AudioRuninCmd.exe 进行音频测试，测试指定时间长度，测试结束后备份结果和调试日志
// 2.7 指纹测试，使用 FingerFactory.CreateDevice().RuningTest() 执行指纹测试，不同的厂商调用不同的程序测试，测试指定时间长度
// 2.8 触控板测试，使用 ClickpadFactory.CreateDevice().RuningTest() 执行触控板测试，不同的厂商调用不同的程序测试，测试指定时间长度
// 2.9 触摸屏测试，将触屏自检工具拷贝到桌面（供应商要求），启动 FT_MultipleTest.exe 进行触摸屏自检，循环执行测试直到时间结束，备份测试结果日志
// 2.10 摄像头测试，启动 camera_burn.exe 进行摄像头测试，测试结束后检查结果文件，支持失败重试机制
// 2.11 SSD和内存烧机测试，根据配置选择合适的测试配置文件，启动 MfwCore.exe 进行SSD和内存测试，监控测试进程直到完成，检查日志文件判断测试结果
// 3. 测试监控和同步
// 3.1 进程状态监控，WndProc()，通过窗口消息处理各个测试进程的状态更新，当进程结束时更新 ListProcess 中的测试状态
// 3.2 等待所有测试完成，超时时间为配置时间+3分钟
// 3.3 检查是否有未完成的测试进程，必要时等待或超时处理
// 4. 结果汇总和清理
// 4.1 检查所有测试进程的状态，存在失败或未完成的测试则整体测试失败
// 4.2 恢复窗口状态和电源配置
// 4.3 重启气泡屏保程序
```
- ColdBootTest
...
- WarmBootTest
```C#
// 1. 从注册表读取当前测试圈数(CurrentCycle)；如果读取失败或圈数小于1，则重置为1；重置注册表中的驱动失败计数(DriverFailCount)为0
// 2. 热启动时长检查
// 2.1 获取系统启动时间SysBootTime赋值给_EndTime
// 2.2 从注册表中读取开始时间赋值给_StartTime
// 2.3 计算冷启动周期时间_EndTime - _StartTime是否超出规格，如果超出则fail
// 3. 检查AC电源状态CheckACState()
// 4. 检查是否完成所有测试圈数
// 4.1 如果当前圈数大于配置的总圈数，测试完成
// 5. 检查设备黄色标记，具体逻辑参见ColdBootOneTimeTest
// 6. 设置自动运行, 保存当前圈数到注册表, 保存开始时间到注册表
// 7. 执行WB.bat批处理文件重启
```
- CPUTest
```C#
...
```
- S4Test，挂起到硬盘/休眠是一种电源管理测试，验证系统能否正常进入和从休眠状态恢复
```C#
// 1. 检查AC电源是否连接, 无AC电源则测试失败
// 2. 如果第一次运行，设置当前圈数为1，重置驱动失败计数器为0，重启计算机一次（确保干净的初始状态）
// 3. 使用 do-while(true) 循环执行测试，直到满足退出条件
// 3.1 黄标检查
// 3.2 透过EC指令设置冷启动测试模式和时间
// 3.3 系统命令 shutdown.exe -h 进入休眠状态
// 3.4 如果之前设置了冷启动模式，则在休眠后退出该模式
// 3.5 检查循环时间是否超时
...
- 控制器，设定IP，并控制灯带
- 服务器，安装数据库和控灯程序
- 交换机，用于连接机台和服务器，以及控制器
- 网线，必须按照灯带的顺序连接，1号网线控制的灯必须连接交换机的1号口

#### 基本原理
- 机台获取所连网线的MAC地址，并将机台的信息和MAC，以及灯的状态信息发送给数据库
- 数据库（会绑定交换机IP和控制盒子IP，以及灯带的布局等信息）会对获取到的信息做一些预处理
- 控灯程序基于数据库中的信息对灯带进行控制（红绿黄）
- 有MAC地址就知道是哪个交换机的哪个口的需求，再基于交换机和控制盒绑定关系，就可以控制对应的控制盒的灯亮
...
flowchart TD
    subgraph 硬件组件
        L[控制器] --> K[灯带]
        N --> L
        N --> M[服务器，会绑定交换机IP和控制盒子IP，以及灯带的布局等信息==>基于交换机和控制盒绑定关系，就可以控制对应的控制盒的灯亮]
        M --> N[交换机]
        O[网点] --> N
        U[机台，获取MAC地址==>有MAC地址就知道是哪个交换机的哪个口的需求] --> O
    end
```
